[![wercker status](https://app.wercker.com/status/869c3599ae2b088beafa8aeca93b1483/s/master "wercker status")](https://app.wercker.com/project/byKey/869c3599ae2b088beafa8aeca93b1483)

# [TutsWiki.com](https://tutswiki.com)
Collaborative tutorials for the internet

![collaborate](https://github.com/TutsWiki/source/blob/master/static/img/collaborate.png "Collaborate")

# Tutorials
- [Pandas Cookbook](https://tutswiki.com/pandas-cookbook/)

# Articles
- [Python](https://tutswiki.com/tags/python/)
  - [How to convert a Python script to module?](https://tutswiki.com/convert-python-script-to-module/)
  - [Abstract classes and interfaces in Python](https://tutswiki.com/abstract-classes-and-interfaces-in-python/)
  - [What is the use of yield in Python?](https://tutswiki.com/what-is-the-use-of-yield-in-python/)
  - [Difference between append and extend in Python](https://tutswiki.com/append-vs-extend-python/)
  - [How to print on same line with print in Python?](https://tutswiki.com/print-same-line-python/)
  - [Writing and Reading config files in Python](https://tutswiki.com/read-write-config-files-in-python/)
  - [How to run a Python module as script?](https://tutswiki.com/run-module-as-script-python/)
  - [What is if __name__ == "__main__" in Python?](https://tutswiki.com/if-name-main-in-python/)

- [Angular](https://tutswiki.com/tags/angular/)
  - [Angular 6 Features](https://tutswiki.com/angular6-features/)
  - [Writing and Reading config files in Angular](https://tutswiki.com/read-write-config-files-in-angular/)
  
- [Other](https://tutswiki.com/blog/)
  - [Approximating Randomness](https://tutswiki.com/approximating-randomness/)
  - [Grunt Tutorial with Live Example](https://tutswiki.com/grunt-tutorial-example/)
  - [noprocrast](https://tutswiki.com/noprocrast/)
  - [sudo node command not found](https://tutswiki.com/how-to-fix-sudo-node-command-not-found-error/)
  - [monad tutorial](https://tutswiki.com/yet-another-lousy-monad-tutorial/)
  - [Ruby enumerable](https://tutswiki.com/ruby-enumerable/)
  - [Rest API in clojure](https://tutswiki.com/rest-api-in-clojure/)
  - [How Deep Learning Helped Reducing Variability in Cardiovascular Imaging](https://tutswiki.com/deep-learning-cardiovascular-imaging/)
  - [Google Engineers Boycott Against Security Tool for Military](https://tutswiki.com/google-engineers-boycott-security-tool-military/)
  - [How to deploy Node.js app on Google App Engine](https://tutswiki.com/nodejs-google-app-engine/)
  - [GitHub Alternatives (Free, Paid, Self-Hosted)](https://tutswiki.com/github-alternatives/)
